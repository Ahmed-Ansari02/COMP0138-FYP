#!/bin/bash

# Configuration
WASI_SDK_PATH="/opt/wasi-sdk"
CC="${WASI_SDK_PATH}/bin/clang"
WAMRC="../../Wasm_controller/compilers/wamrc"
WASM_DIR="../wasm_assets/wasm"
AOT_DIR="../wasm_assets/aot"
SCALING_DIR="../wasm_assets/scaling"

# Ensure output directories exist
mkdir -p "$WASM_DIR"
mkdir -p "$AOT_DIR"
mkdir -p "$SCALING_DIR"

# Compilation flags
CFLAGS="-O3 \
    --target=wasm32-wasi \
    -nostdlib \
    -I../main \
    -Wl,--initial-memory=65536 \
    -Wl,--export=__heap_base,--export=__data_end \
    -z stack-size=2048 \
    -Wl,--no-entry \
    -Wl,--export=main \
    -Wl,--export=produce,--export=consume \
    -Wl,--allow-undefined "

SCALING_CFLAGS="-Oz \
    --target=wasm32-wasi \
    -nostdlib \
    -I../main \
    -ffunction-sections \
    -fdata-sections \
    -Wl,--initial-memory=65536 \
    -Wl,--export=__heap_base,--export=__data_end \
    -z stack-size=2048 \
    -Wl,--strip-all \
    -Wl,--gc-sections \
    -Wl,--no-entry \
    -Wl,--export=main \
    -Wl,--allow-undefined "

compile_file() {
    local input_file="$1"
    local filename=$(basename -- "$input_file")
    local name="${filename%.*}"
    local wasm_file="$WASM_DIR/$name.wasm"
    local aot_file="$AOT_DIR/$name.aot"
    local compile_flags="$CFLAGS"

    if [ "$name" = "scaling_worker" ]; then
        compile_flags="$SCALING_CFLAGS"
    fi

    # Step 1: C -> WASM
    echo "[WASM] $input_file -> $wasm_file"

    if [ ! -x "$CC" ]; then
        echo "Error: Compiler not found at $CC"
        echo "Please ensure WASI SDK is installed at $WASI_SDK_PATH"
        exit 1
    fi

    "$CC" $compile_flags -DPAYLOAD_SIZE=${PAYLOAD_SIZE:-0} -o "$wasm_file" "$input_file"

    if [ $? -ne 0 ]; then
        echo "Failed to compile $input_file to WASM"
        exit 1
    fi

    # Step 2: WASM -> AOT
    if [ -x "$WAMRC" ]; then
        echo "[AOT]  $wasm_file -> $aot_file"
        "$WAMRC" --target=xtensa --opt-level=3 --size-level=3 -o "$aot_file" "$wasm_file"
        if [ $? -ne 0 ]; then
            echo "Failed to compile $wasm_file to AOT"
            exit 1
        fi
    else
        echo "[AOT]  Skipped (wamrc not found at $WAMRC)"
    fi

    if [ "$name" = "scaling_worker" ]; then
        cp "$wasm_file" "$SCALING_DIR/$name.wasm"
        if [ -f "$aot_file" ]; then
            cp "$aot_file" "$SCALING_DIR/$name.aot"
        fi
    fi
}

if [ -n "$1" ]; then
    if [ -f "$1" ]; then
        compile_file "$1"
    else
        echo "Error: File $1 not found."
        exit 1
    fi
else
    echo "No file specified. Compiling all .c files in current directory..."
    found_files=false
    for file in *.c; do
        if [ -f "$file" ]; then
            compile_file "$file"
            found_files=true
        fi
    done

    if [ "$found_files" = false ]; then
        echo "No .c files found in the current directory."
    fi
fi

echo ""
echo "WASM binaries:"
ls -lh "$WASM_DIR"/*.wasm 2>/dev/null
echo ""
echo "AOT binaries:"
ls -lh "$AOT_DIR"/*.aot 2>/dev/null
